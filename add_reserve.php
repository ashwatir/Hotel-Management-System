...
<head>
    ...
    <style>
        .header{
            background-image: linear-gradient(rgba(0, 0, 0, 0.8), rgba(0, 0, 0, 0.7)), url("photo/<?php echo $fetch['photo']?>");
        }
  ...
  <div class="form">
    <form method = "POST" enctype = "multipart/form-data">
       <div class="inputfield">
          ...
          <input type="text" class="input" name = "firstname" required = "required">
       </div>  
        <div class="inputfield">
          <label>Last Name</label>
          <input type="text" class="input" name = "lastname" required = "required">
       </div>  
        
      <div class="inputfield">
          <label>Phone Number</label>
          <input type="text" class="input" name = "contactno" required = "required">
       </div> 
      <div class="inputfield">
          <label>Address</label>
          <textarea class="textarea" name = "address" required = "required"></textarea>
       </div> 
      <div class="inputfield">
           <label>Check in</label>
			<input type = "date" class = "input" name = "date" required = "required" />
	 </div> 
      ...
      <div class="inputfield">
        <input type="submit" value="Register" class="btn" name = "add_guest">
      </div>
    </div>
</div>
      </form>
  ...
  if(ISSET($_POST['add_guest'])){
    ...
    if($checkin < date("Y-m-d", strtotime('+8 HOURS'))){  
        ...
        if($row > 0){
          ...
          //       <label style = 'color:#ff0000;'>Room Not Available On</label><br />";
